...
describe("student recommendation request", () => {
  ...
  it("keeps the FALLBACK Skill Score explanation accurate when skill coverage is below 100%", () => {
    ...
    return getRecommendationRun("1").then(({ results }) => {
      ...
      expect(reasons).toContain("Skill Score chỉ thể hiện mức độ đáp ứng các kỹ năng đã khai báo của vị trí, không phải độ phù hợp tổng thể của CV với công việc.");
      expect(reasons).not.toContain("Skill Score 100%");
    });
  });
});
